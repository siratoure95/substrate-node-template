...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Directly transfer a dog to another recipient.
        ///
        /// Any account that holds a dog can send it to another Account. This will reset the
        /// asking price of the dog, marking it not for sale.
        #[pallet::weight(0)]
        pub fn transfer_dog(
            origin: OriginFor<T>,
            to: T::AccountId,
            ...
            let dog = Dogs::<T>::get(&dog_id).ok_or(Error::<T>::NoDog)?;
            ensure!(dog.owner == from, Error::<T>::NotOwner);
            Self::do_transfer_dog(dog_id, to)?;
            Ok(())
        }
        ...
        pub fn set_dog_price(
            ...
            // Ensure the kitty exists and is called by the kitty owner
            let mut dog = Dogs::<T>::get(&dog_id).ok_or(Error::<T>::NoDog)?;
            ensure!(dog.owner == sender, Error::<T>::NotOwner);
            ...
            // Deposit a "PriceSet" event.
            Self::deposit_event(Event::PriceSetDog { dog: dog_id, price: new_price });
        ...
        pub fn buy_dog(
            ...
            let buyer = ensure_signed(origin)?;
            // Transfer the kitty from seller to buyer as a sale.
            Self::do_buy_dog(dog_id, buyer, bid_price)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn mint_dog(
            ...
            ensure!(!Dogs::<T>::contains_key(&dog.dna), Error::<T>::DuplicateDog);
        ...
        // Update storage to transfer dog
        pub fn do_transfer_dog(
            dog_id: [u8; 16],
            to: T::AccountId,
        ) -> DispatchResult {
            // Get the dog
            ...
            Self::deposit_event(Event::TransferredDog { from, to, dog: dog_id });
        ...
        pub fn do_buy_dog(
            ...
            // Remove dog from list of owned kitties.
            if let Some(ind) = from_owned.iter().position(|&id| id == dog_id) {
                from_owned.swap_remove(ind);
            } else {
                return Err(Error::<T>::NoDog.into())
            ...
            // Add dog to the list of owned kitties.
            let mut to_owned = DogsOwned::<T>::get(&to);
            to_owned.try_push(dog_id).map_err(|()| Error::<T>::TooManyOwned)?;
            ...
            if let Some(price) = dog.price {
                ...
                T::Currency::transfer(&to, &from, price, frame_support::traits::ExistenceRequirement::KeepAlive)?;
                // Deposit sold event
                Self::deposit_event(Event::SoldDog {
                    seller: from.clone(),
                    buyer: to.clone(),
                    dog: dog_id,
                    price,
            ...
            // Transfer succeeded, update the dog owner and reset the price to `None`.
            dog.owner = to.clone();
            dog.price = None;
